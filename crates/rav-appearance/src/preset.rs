...
/// rav as it has always looked: the transcribed ramp, block glyphs, and a
/// linear response.
...
mod tests {
    ...
    #[test]
    fn the_winamp_preset_changes_nothing_about_rav() {
        ...
        assert_eq!(WINAMP.ballistics, ballistics::WINAMP);
        assert_eq!(WINAMP.curve, Curve::Linear, "rav measures linearly");
        assert_eq!(WINAMP.skin.steps(), 8, "eighth blocks");
    ...
    fn a_strip_preset_departs_only_where_the_hardware_forces_it() {
        ...
        assert_eq!(STRIP.ballistics, WINAMP.ballistics, "the feel is the same");
        assert_eq!(STRIP.theme.name, WINAMP.theme.name, "so are the colours");
        assert_ne!(STRIP.curve, WINAMP.curve, "but not the response");
    ...
    fn the_strip_curve_is_what_makes_four_lights_usable() {
        ...
        assert_eq!(WINAMP.curve.apply(quiet).fill(4, 1).whole, 0);
