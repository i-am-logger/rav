...
impl CyberneticQualityController {
    ...
    pub fn control_loop(&mut self, current_frame_data: &FrameData) -> QualityControlActions {
        ...
        let predicted_issues = self.predictor.predict_quality_issues(
        ...
        if quality_assessment.needs_immediate_action() {
            actions = self.optimizer.generate_corrective_actions(&quality_assessment);
            self.corrective_actions_taken += 1;
        }
        
        // Handle predicted future issues (forward loop)
        if !predicted_issues.is_empty() {
            let preventive_actions = self.optimizer.generate_preventive_actions(&predicted_issues);
    ...
    fn compare_against_targets(&self) -> QualityAssessment {
        ...
        let animation_gap = self.animation_smoothness_target - self.current_metrics.animation_smoothness;
        let audio_gap = self.audio_responsiveness_target - self.current_metrics.audio_responsiveness;
    ...
    fn calculate_quality_trend(&self) -> QualityTrend {
        ...
        let older_avg = recent_metrics[5..].iter()
            .map(|m| m.overall_score())
            .sum::<f32>() / 5.0;
        let recent_avg = recent_metrics[..5].iter()
            .map(|m| m.overall_score())
            .sum::<f32>() / 5.0;
    ...
    fn calculate_urgency_level(&self, color_gap: f32, animation_gap: f32, audio_gap: f32) -> UrgencyLevel {
    ...
    fn calculate_target_achievement(&self) -> f32 {
        let color_achievement = (self.current_metrics.color_variety / self.color_variety_target).min(1.0);
        let animation_achievement = (self.current_metrics.animation_smoothness / self.animation_smoothness_target).min(1.0);
        let audio_achievement = (self.current_metrics.audio_responsiveness / self.audio_responsiveness_target).min(1.0);
...
impl QualityAssessment {
    pub fn needs_immediate_action(&self) -> bool {
        matches!(self.urgency_level, UrgencyLevel::Critical | UrgencyLevel::High)
    ...
    pub fn overall_improvement(&self) -> f32 {
        ...
        let total_gap = self.color_variety_gap + self.animation_smoothness_gap + self.audio_responsiveness_gap;
...
impl QualityControlActions {
    ...
    pub fn merge_preventive(&mut self, preventive: QualityControlActions) {
        self.color_enhancements.extend(preventive.color_enhancements);
        self.animation_adjustments.extend(preventive.animation_adjustments);
        self.audio_optimizations.extend(preventive.audio_optimizations);
