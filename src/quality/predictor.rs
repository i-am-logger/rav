...
impl QualityPredictor {
    ...
    fn predict_color_degradation(
        ...
        let values: Vec<f32> = recent_metrics.iter().map(|m| m.color_variety).collect();
    ...
    fn predict_animation_stutter(
        ...
        let values: Vec<f32> = recent_metrics
            .iter()
    ...
    pub fn update_accuracy(&mut self, predicted_issues: &[PredictedIssue], actual_outcome: f32) {
        ...
        let average_confidence: f32 = predicted_issues
            .iter()
            .map(|issue| issue.confidence)
            .sum::<f32>()
            / predicted_issues.len() as f32;
    ...
    fn update_model_parameters(&mut self, accuracy: f32) {
        ...
        self.color_degradation_model
            .update_sensitivity(adjustment_factor);
        self.animation_stutter_model
            .update_sensitivity(adjustment_factor);
        self.audio_latency_model
            .update_sensitivity(adjustment_factor);
