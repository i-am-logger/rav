...
impl VisualQualityMetrics {
    ...
    fn analyze_color_quality(&mut self, colors: &[(u8, u8, u8)]) {
        ...
        self.color_variety = self.color_variety * (0.5 + 0.5 * self.color_spectrum_coverage);
    }
    ...
    fn calculate_spectrum_coverage(&self, colors: &[(u8, u8, u8)]) -> f32 {
        ...
        let mut hue_bins = vec![false; 12]; // 12 hue regions (30° each)
    ...
    pub fn smooth_with(&self, new_metrics: &VisualQualityMetrics, smoothing_factor: f32) -> Self {
        ...
        VisualQualityMetrics {
            ...
            animation_smoothness: beta * self.animation_smoothness + alpha * new_metrics.animation_smoothness,
            audio_responsiveness: beta * self.audio_responsiveness + alpha * new_metrics.audio_responsiveness,
            render_performance: beta * self.render_performance + alpha * new_metrics.render_performance,
            timestamp: new_metrics.timestamp,
            
            colors_detected: new_metrics.colors_detected,
            color_spectrum_coverage: beta * self.color_spectrum_coverage + alpha * new_metrics.color_spectrum_coverage,
            frame_time_variance: beta * self.frame_time_variance + alpha * new_metrics.frame_time_variance,
            audio_peak_response: beta * self.audio_peak_response + alpha * new_metrics.audio_peak_response,
            visual_complexity: beta * self.visual_complexity + alpha * new_metrics.visual_complexity,
        }
    }
    ...
    pub fn detailed_report(&self) -> String {
        format!(
            ...
            if self.meets_production_quality() { "✅ Yes" } else { "❌ No" }
        )
    }
}
